...
function BuildStageProblem(InputParameters::InputParam, SolverParameters::SolverParam, Battery::BatteryParam)       #, state_variables::states When we have 2 hydropower plants- 2 turbines
    ...
    #@variable(M, min_SOH <= soh_final <= max_SOH, base_name = "Final_Capacity")        #energy_Capacity     [iStage=1:NStages]
    #@variable(M, min_SOH <= soh_new <= max_SOH, base_name = "Initial_Capacity")     #energy_Capacity      [iStage=1:NStages]
    ...
    @objective(
      ...
      #Battery_price[1]*(soh_new-min_SOH) + 
      #Battery_price[2]*(soh_final-min_SOH) 
...
#=
    ...
    @constraint(M,soh, soh_new >= soh_final)      #soh[iStage=1:(NStages-1)]

    @constraint(M,final_soh, soh_final == soh_new - sum(deg[iStep] for iStep=1:NSteps)*k )     #deg2
