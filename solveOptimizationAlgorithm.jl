...
function solveOptimizationProblem(InputParameters::InputParam, SolverParameters::SolverParam, Battery::BatteryParam)
    ...
    @timeit to "Collecting results" begin
        ...
        for iStage=1:NStages
            soh_final = JuMP.value(problem.soh_final)
            soh_initial = JuMP.value(problem.soh_new)
        ...
        revenues_per_stage[iStage] = sum(Power_prices[iStep]*NHoursStep*max_P*(discharge[iStep]-charge[iStep]) for iStep=((iStage-1)*NHoursStage+1):(NHoursStage*iStage)) #- Battery_price[1]*(soh_initial-min_SOH) + Battery_price[2]*(soh_final-min_SOH)
        ...
        #cost_rev[iStage] = Battery_price[1]*(soh_initial-min_SOH) - Battery_price[2]*(soh_final-min_SOH)
        end
